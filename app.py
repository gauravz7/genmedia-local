...
def get_gcloud_token():
    try:
        ...
        print(f"Error getting gcloud token: {e}. Make sure gcloud CLI is installed and you are authenticated.")
        return None
...
def generate_image_video_internal(app_context, prompt, operation_id, image_bytes, model_name, seed, aspect_ratio, negative_prompt):
    with app_context:
        ...
        if not token:
            history_item.status = 'failed'
            history_item.error_message = "Failed to get gcloud authentication token."
            db.session.commit()
            return
...
# Also update your imports at the top of app.py - remove the monitoring import:
# Remove this line: from google.cloud import monitoring
# Keep this line: import datetime (if not already present)
